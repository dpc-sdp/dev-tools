...
drush php-eval "\$prefix = getenv('DRUPAL_MODULE_PREFIX'); if (\$prefix) { module_load_install(\$prefix . '_core'); \$func = \$prefix . '_core_default_content_modules'; \$func(); }"
echo "==> Enabling site-specific modules"
drush php-eval "\$prefix = getenv('DRUPAL_MODULE_PREFIX'); if (\$prefix) { module_load_install(\$prefix . '_core'); \$func = \$prefix . '_core_enable_modules'; \$func(); }"
drush cr -y
